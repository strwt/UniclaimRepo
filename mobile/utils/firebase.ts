...
export const messageService = {
    ...
    async updateHandoverResponse(conversationId: string, messageId: string, status: 'accepted' | 'rejected', responderId: string, idPhotoUrl?: string): Promise<void> {
        try {
            ...
            if (status === 'rejected') {
                console.log('🗑️ Mobile: Handover rejected, starting photo deletion process...');
                try {
                    // Step 1: Extract all photos from the handover message
                    const messageDoc = await getDoc(messageRef);
                    if (messageDoc.exists()) {
                        const messageData = messageDoc.data();
                        console.log('🗑️ Mobile: Message data retrieved:', messageData);
                        const imageUrls = extractMessageImages(messageData);
                        console.log('🗑️ Mobile: Extracted image URLs:', imageUrls);

                        // Step 2: Delete photos from Cloudinary
                        if (imageUrls.length > 0) {
                            try {
                                await deleteMessageImages(imageUrls);
                                console.log('✅ Mobile: Photos deleted after handover rejection:', imageUrls.length);
                    ...
                    // Reset the handoverRequested flag to allow new requests
                    const conversationRef = doc(db, 'conversations', conversationId);
                    await updateDoc(conversationRef, {
                    ...
                    console.warn('⚠️ Mobile: Failed to cleanup photos after rejection:', error.message);
                    // Continue with rejection even if photo cleanup fails
                }
            }
                    ...
                    if (messageSnap.exists()) {
                        ...
                        if (handoverData) {
                            ...
                            if (handoverPersonDoc.exists()) {
                                ...
                                console.log('✅ Mobile: Post updated with complete handover details and conversation data:', postId);
                                console.log('✅ Mobile: Handover request chat bubble details preserved in post');

                                // STEP 7: Delete the conversation after successful data preservation
                                try {
                                    console.log('🗑️ Mobile: Starting conversation deletion after successful handover confirmation...');
                                    ...
                                    console.warn('⚠️ Mobile: Failed to delete conversation after handover confirmation:', deletionError.message);
        ...
        try {
            ...
            try {
                ...
                if (!messagesSnapshot.empty) {
                    ...
                    console.log('🔄 Mobile: Updated conversation lastMessage after deletion');
                } else {
                    // No messages left, clear the lastMessage
                    await updateDoc(conversationRef, {
                        lastMessage: null
                    });
                    console.log('🗑️ Mobile: Cleared conversation lastMessage - no messages remaining');
                }
            } catch (updateError: any) {
                console.warn('Mobile: Failed to update conversation lastMessage after deletion:', updateError.message);
...
export const postService = {
    ...
    async updatePostStatus(postId: string, status: 'pending' | 'resolved' | 'unclaimed'): Promise<void> {
        try {
            ...
            if (status === 'unclaimed') {
                console.log(`🗑️ Mobile: Post marked as unclaimed, deleting all related conversations for post: ${postId}`);
                await this.deleteConversationsByPostId(postId);
                console.log(`✅ Mobile: Successfully deleted conversations for unclaimed post: ${postId}`);
            }
        } catch (error: any) {
    ...
    async cleanupHandoverDetailsAndPhotos(postId: string): Promise<{ photosDeleted: number; errors: string[] }> {
        try {
            ...
            if (!handoverDetails) {
                console.log('ℹ️ Mobile: No handover details found, nothing to clean up');
            ...
            if (handoverDetails.handoverItemPhotos && Array.isArray(handoverDetails.handoverItemPhotos)) {
                handoverDetails.handoverItemPhotos.forEach((photo: any) => {
                    if (photo.url && typeof photo.url === 'string' && photo.url.includes('cloudinary.com')) {
                        ...
                        console.log(`🗑️ Mobile: Found handover item photo for deletion: ${photo.url.split('/').pop()}`);
                    }
                });
            }
            ...
            if (handoverDetails.handoverIdPhoto && typeof handoverDetails.handoverIdPhoto === 'string' && handoverDetails.handoverIdPhoto.includes('cloudinary.com')) {
                ...
                console.log(`🗑️ Mobile: Found handover ID photo for deletion: ${handoverDetails.handoverIdPhoto.split('/').pop()}`);
            }
            ...
            if (handoverDetails.ownerIdPhoto && typeof handoverDetails.ownerIdPhoto === 'string' && handoverDetails.ownerIdPhoto.includes('cloudinary.com')) {
                ...
                console.log(`🗑️ Mobile: Found owner ID photo for deletion: ${handoverDetails.ownerIdPhoto.split('/').pop()}`);
            }
            ...
            console.log(`✅ Mobile: Handover details cleared from post: ${postId}`);
