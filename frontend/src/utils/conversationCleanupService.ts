...
export class ConversationCleanupService {
    ...
    public async cleanupOldConversations(): Promise<CleanupResult> {
        ...
        if (now - this.lastCleanupTime < this.MIN_CLEANUP_INTERVAL) {
            console.log('🧹 Cleanup interval not reached, skipping...');
            return {
                conversationsDeleted: 0,
                messagesDeleted: 0,
                imagesDeleted: 0,
        ...
        try {
            console.log('🧹 Starting 24-hour conversation cleanup...');

            const result = await this.performCleanup();

            console.log(`✅ Cleanup completed: ${result.conversationsDeleted} conversations, ${result.messagesDeleted} messages, ${result.imagesDeleted} images deleted`);
    ...
    private async performCleanup(): Promise<CleanupResult> {
        ...
        try {
            ...
            console.log(`🧹 Found ${oldConversations.length} conversations older than 24 hours`);
            ...
            for (const conversation of oldConversations) {
                try {
                    ...
                    console.log(`✅ Cleaned up conversation ${conversation.id}: ${conversationResult.messagesDeleted} messages, ${conversationResult.imagesDeleted} images`);
                } catch (error: any) {
    ...
    private async deleteMessageImages(messageData: MessageData): Promise<number> {
        ...
        try {
            ...
            console.warn(`⚠️ Error during image cleanup for message:`, error.message);
        }
